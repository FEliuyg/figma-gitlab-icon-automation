...
export default ({ hidden, onFilled, webhookData }) => {
  ...
  React.useEffect(() => {
    if (!visible) {
      onFilled();
      parent.postMessage(
        { pluginMessage: { type: "setWebhookData", webhookData: "" } },
        "*"
      );
    }
  ...
  React.useEffect(() => {
    const { webhookUrl: url, data: message } = webhookData || {
      webhookUrl: "",
      data: "",
    };
    if (url && message) {
      ...
      <React.Fragment>
        <div className="checkbox">
          <input
            ...
            checked={visible}
          />
          <label htmlFor="visible" className="checkbox__label">
            Send a message to Slack/Lark
          </label>
        </div>
        <div className={visible ? "" : "hide"}>
          ...
          <div className="form-item">
            <textarea
              ...
              onChange={handleChange}
            />
            <div className="type type--pos-medium-normal">
              You can use variables $prUrl, $version, $message in the content.
            </div>
          </div>
        </div>
      </React.Fragment>
